...
def run_all_exp(conn, ID):
    ...
    res = []
    res = [mongoQueries.basic_selects(conn),mongoQueries.conditional_selects(conn)]
    #res.append(mongoQueries.conditional_selects(conn))
    res.append(mongoQueries.update(conn))
...
def test():
    for NUM_THREADS in [2, 4, 6, 8, 10, 12, 14]:
        ...
        final_times = []
        create_threads_and_run(mongoConnect.xs_db, NUM_THREADS)
        build_all_data('XS', NUM_THREADS)
        
        final_times = []
        create_threads_and_run(mongoConnect.s_db, NUM_THREADS)
        build_all_data('S', NUM_THREADS)
        
        final_times = []
        create_threads_and_run(mongoConnect.m_db, NUM_THREADS)
        build_all_data('M', NUM_THREADS)
        
        final_times = []
        create_threads_and_run(mongoConnect.l_db, NUM_THREADS)
